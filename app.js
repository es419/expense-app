...
document.getElementById("exp-save").addEventListener("click", async () => {
  ...
  await db.collection("users").doc(state.user.uid).collection("expenses").add({
    amount, categoryId: selectedExpCat, date, note,
...
function exportAllToExcel() {
  ...
  const overviewRows = [["חודש", "מס' הוצאות", "סה\"כ הוצאה (₪)"]];
  monthKeys.forEach(key => {
    const rows = byMonth[key];
    const total = rows.reduce((s, e) => s + Number(e.amount), 0);
    overviewRows.push([monthKeyToLabel(key), rows.length, Math.round(total)]);
  });
  const overviewWs = XLSX.utils.aoa_to_sheet(overviewRows);
  overviewWs["!cols"] = [{ wch: 16 }, { wch: 12 }, { wch: 16 }];
  XLSX.utils.book_append_sheet(wb, overviewWs, "סיכום כללי");

  // ---- one sheet per month ----
  monthKeys.forEach(key => {
    const rows = byMonth[key].slice().sort((a, b) => a.date.localeCompare(b.date));
    const sheetRows = [["תאריך", "קטגוריה", "סכום (₪)", "הערה"]];
    rows.forEach(e => {
      sheetRows.push([e.date, catById(e.categoryId).name, Number(e.amount), e.note || ""]);
    });
    ...
    ws["!cols"] = [{ wch: 12 }, { wch: 16 }, { wch: 12 }, { wch: 24 }];
