...
function renderAll() {
  if (!state.user) return;
  renderMonthLabel();
  if (!state.categoriesLoaded || !state.expensesLoaded) {
    renderSkeletons();
    return;
  }
  renderReceipt();
...
function renderSkeletons() {
  // receipt
  document.getElementById("total-spent").innerHTML = `<span class="skel" style="display:inline-block;width:100px;height:32px;"></span>`;
  document.getElementById("total-budget").innerHTML = `<span class="skel" style="display:inline-block;width:40px;height:14px;vertical-align:middle;"></span>`;
  document.getElementById("total-remaining-wrap").innerHTML = `<span class="skel" style="display:inline-block;width:70px;height:14px;vertical-align:middle;"></span>`;
  document.getElementById("total-bar").style.width = "0%";
  document.getElementById("stat-count").innerHTML = `<span class="skel" style="display:inline-block;width:22px;height:16px;"></span>`;
  document.getElementById("stat-avg").innerHTML = `<span class="skel" style="display:inline-block;width:34px;height:16px;"></span>`;
  document.getElementById("stat-top").innerHTML = `<span class="skel" style="display:inline-block;width:48px;height:16px;"></span>`;

  // category list
  document.getElementById("category-list").innerHTML = Array.from({ length: 4 }).map(() => `
    <div class="skel-row">
      <span class="skel skel-dot"></span>
      <div style="flex:1">
        <div class="skel" style="width:55%; height:12px; margin-bottom:8px;"></div>
        <div class="skel" style="width:100%; height:6px;"></div>
      </div>
    </div>`).join("");

  // expense list
  document.getElementById("expense-list").innerHTML = Array.from({ length: 5 }).map(() => `
    <div class="skel-row">
      <span class="skel" style="width:34px;height:34px;border-radius:10px;"></span>
      <div style="flex:1">
        <div class="skel" style="width:45%; height:12px; margin-bottom:6px;"></div>
        <div class="skel" style="width:28%; height:9px;"></div>
      </div>
      <span class="skel" style="width:50px;height:13px;"></span>
    </div>`).join("");

  // chart
  document.getElementById("chart-canvas").style.display = "none";
  document.getElementById("chart-legend").innerHTML = `<div class="skel" style="width:100%; height:200px;"></div>`;
}
...
function renderReceipt() {
  ...
  document.getElementById("total-remaining").textContent = fmtNum(Math.abs(remaining));
  ...
  animateIn(document.querySelector(".receipt"));
}
...
function renderChart() {
  ...
  if (state.chartMode === "payment") {
    ...
    animateIn(document.getElementById("chart-card"));
    return;
  }
  ...
  if (state.chartMode === "pie") {
    ...
    chartInstance = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          ...
          { label: "תקציב", data: budget, backgroundColor: "#E4DFD1", borderRadius: 6, maxBarThickness: 26 },
        ]
      },
      options: {
    ...
    legend.innerHTML = `
      <div class="legend-item"><span class="legend-dot" style="background:#6E685D"></span>בפועל</div>
      <div class="legend-item"><span class="legend-dot" style="background:#E4DFD1"></span>תקציב</div>
    `;
  }
  animateIn(document.getElementById("chart-card"));
}
...
function renderPaymentChart(ctx, legend, monthExpenses) {
  ...
  chartInstance = new Chart(ctx, {
    ...
    data: {
      labels: ["אשראי", "מזומן"],
      datasets: [{ data: [totals.credit, totals.cash], backgroundColor: [colors.credit, colors.cash], borderWidth: 2, borderColor: "#FFFFFF" }],
    },
    options: {
